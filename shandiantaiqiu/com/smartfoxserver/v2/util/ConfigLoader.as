package com.smartfoxserver.v2.util
{
   ...
   public class ConfigLoader extends EventDispatcher
   {
      ...
      private function onConfigLoadSuccess(evt:Event) : void
      {
         ...
         cfgData.host = cfgData.bboxHost = xmlDoc.ip;
         cfgData.port = int(xmlDoc.port);
         cfgData.udpHost = xmlDoc.udpIp;
         cfgData.udpPort = int(xmlDoc.udpPort);
         cfgData.zone = xmlDoc.zone;
         if(xmlDoc.blueBoxAddress != undefined)
         {
            cfgData.bboxHost = xmlDoc.blueBoxAddress;
         }
         if(xmlDoc.blueBoxPort != undefined)
         {
            cfgData.bboxPort = xmlDoc.blueBoxPort;
         }
         ...
         {
            cfgData.useBBox = xmlDoc.useBlueBox.toLowerCase() == "true" ? Boolean(true) : Boolean(false);
         }
         ...
         {
            cfgData.bboxPollingRate = int(xmlDoc.blueBoxPollingRate);
         }
